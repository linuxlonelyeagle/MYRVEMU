...
#define unreachable() (fatal("unreadchable"), __builtin_unreachable())
...
u64 mmu_alloc(mmu_t *, int);

inline void mmu_writer(u64 addr, u8* data, size_t len) {
...
typedef struct {
    ...
    u64 reenter_pc;
    u64 gp_regs[num_gp_regs]; // risc-v 32 个通用寄存器
